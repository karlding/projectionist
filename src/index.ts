...
function getDbPath(dbName: string): string {
  ...
  throw new Error(`${dbName} not found. Tried: ${candidates.join(", ")}`);
}
...
ipcMain.handle(
  "db:getSongData",
  (_event, sourceSkid: number, sourceSequenceNbr: number) => {
    return getQueries().getSongData(
      Number(sourceSkid),
      Number(sourceSequenceNbr),
    );
  },
);
